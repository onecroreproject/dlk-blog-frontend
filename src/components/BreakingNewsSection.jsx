        ...
        <div className="flex flex-col gap-8">
          {loading ? (
            ...
            leftPosts.map((post) => (
              <Link to={`/blog/${post.slug || post._id}`} key={post._id} className="flex flex-col md:flex-row gap-6 group cursor-pointer bg-white border border-gray-100 rounded-2xl overflow-hidden hover:shadow-xl transition-all duration-300">
                <div className="md:w-2/5 h-48 md:h-auto bg-gray-200 overflow-hidden relative">
                  ...
                  <div className="absolute top-4 left-4 px-3 py-1 bg-red-600 text-white text-xs font-black  rounded-full shadow-lg">
                    {post.category}
                  </div>
                </div>
            ...
            <div className="text-center py-20 bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200">
              <p className="text-gray-400 font-black  text-sm">No News Articles Found</p>
            </div>
          )}

          {/* Load More Button */}
          <div className="mt-4">
            <Link to="/categories/news" className="block text-center w-full py-4 bg-green-500 border border-green-600 rounded-2xl text-white font-black  text-sm hover:bg-green-600 transition-all shadow-sm">
              View All News
            </Link>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-8">
          {loading ? (
            ...
            <div className="flex-1 min-h-[350px] flex items-center justify-center bg-gray-100 rounded-3xl border-2 border-dashed border-gray-200">
              <p className="text-gray-400 font-black  text-sm text-center">Stay tuned for<br />latest news</p>
            </div>
          )}
        </div>
