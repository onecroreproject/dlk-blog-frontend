          ...
          <div className="lg:col-span-4 flex flex-col gap-10">
              ...
              <div className="flex flex-col gap-4">
                {categoryCounts.map((cat, i) => (
                  <Link to={`/category/${cat.name.toLowerCase().replace(/ /g, '-')}`} key={i} className="flex items-center justify-between group cursor-pointer border-b border-gray-50 pb-3 last:border-0">
                    ...
                    <span className="w-8 h-8 rounded-full bg-gray-50 flex items-center justify-center text-sm font-black text-gray-400 group-hover:bg-red-600 group-hover:text-white transition-all shadow-sm">{cat.count}</span>
                  </Link>
                ))}
              </div>
            </div>
            ...
            <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm">
              ...
              <div className="flex flex-wrap gap-2">
                {allTags.length > 0 ? allTags.map((tagItem, i) => (
                  <Link 
                    key={i} 
                    to={`/tag/${tagItem}`}
                    className={`px-4 py-2 rounded-full text-xs font-black tracking-wider cursor-pointer transition-all border border-gray-100 shadow-sm ${tagItem.toLowerCase() === tag.toLowerCase() ? 'bg-red-600 text-white' : 'bg-gray-50 text-gray-400 hover:bg-red-600 hover:text-white'}`}
                  >
                    #{tagItem}
                  </Link>
                )) : (
